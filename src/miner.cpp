...
// ***TODO*** ScanHash is not yet used in Helium
//
...
void static BitcoinMiner(const CChainParams& chainparams)
{
        ...
        while (true) {
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    if (!fvNodesEmpty && !IsInitialBlockDownload()) // && masternodeSync.IsSynced()) <- should check for -litemode
                    {
                        break;
                    }
                    MilliSleep(1000);
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HeliumMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HeliumMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HeliumMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HeliumMiner -- runtime error: %s\n", e.what());
        return;
    }
}
