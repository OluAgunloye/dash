...
const char * const BITCOIN_CONF_FILENAME = "helium.conf";
const char * const BITCOIN_PID_FILENAME = "heliumd.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HeliumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HeliumCore
    // Mac: ~/Library/Application Support/HeliumCore
    // Unix: ~/.heliumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HeliumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HeliumCore";
#else
    // Unix
    return pathRet / ".heliumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty helium.conf if it does not excist
